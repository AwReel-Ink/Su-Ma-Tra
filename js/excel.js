      ...
      return new Promise(function(resolve) {
        ...
        Promise.all([
          ...
          // === Feuille STATS ===
              ...
              // === Feuille GRAPHIQUES ===
              var sheetGraph = workbook.addWorksheet(('GRAPH_' + Capitalize(worker.nom)).substring(0, 31));

              // Titre
              sheetGraph.getCell(1, 1).value = 'GRAPHIQUES — ' + worker.nom.toUpperCase();
              sheetGraph.getCell(1, 1).font = { bold: true, size: 14 };

              // ── Données pour les graphiques (tableau source) ──
              sheetGraph.getCell(3, 1).value = 'Matériel / Tâche';
              sheetGraph.getCell(3, 2).value = 'Minutes';
              sheetGraph.getCell(3, 3).value = '% (hors absences)';
              [1,2,3].forEach(function(col) {
                sheetGraph.getCell(3, col).font = { bold: true };
                sheetGraph.getCell(3, col).fill = {
              ...
              var graphRow = 4;
              machinesSorted.forEach(function(m) {
                sheetGraph.getCell(graphRow, 1).value = m;
                sheetGraph.getCell(graphRow, 2).value = machinesCount[m];
                sheetGraph.getCell(graphRow, 3).value = totalMinutes > 0
                  ? parseFloat(((machinesCount[m] / totalMinutes) * 100).toFixed(1))
                  : 0;
                graphRow++;
              });
              // Absent & Congé
              if (totalAbsent > 0) {
                sheetGraph.getCell(graphRow, 1).value = 'Absent';
                sheetGraph.getCell(graphRow, 2).value = totalAbsent;
                sheetGraph.getCell(graphRow, 3).value = 0;
                graphRow++;
              }
              if (totalConge > 0) {
                sheetGraph.getCell(graphRow, 1).value = 'Congé';
                sheetGraph.getCell(graphRow, 2).value = totalConge;
                sheetGraph.getCell(graphRow, 3).value = 0;
                graphRow++;
              }

              var dataLastRow = graphRow - 1;
              var dataCount   = dataLastRow - 3; // nb de lignes de données

              // ── Camembert (Pie) ──
              var pieChart = workbook.addChart('pie', {
                title:  { name: 'Répartition du temps (camembert)' },
                legend: { position: 'r' }
              });
              pieChart.addSeries({
                name:       sheetGraph.getCell(1, 2).address,
                categories: { sheet: sheetGraph, fromRow: 4, fromCol: 1, toRow: dataLastRow, toCol: 1 },
                values:     { sheet: sheetGraph, fromRow: 4, fromCol: 2, toRow: dataLastRow, toCol: 2 }
              });
              pieChart.setPosition(sheetGraph, 1, 6);
              pieChart.setSize(500, 280);
              sheetGraph.addChart(pieChart);

              // ── Colonnes (Bar) ──
              var barChart = workbook.addChart('bar', {
                title:  { name: 'Temps par matériel (colonnes)' },
                legend: { position: 'b' }
              });
              barChart.addSeries({
                name:       'Minutes',
              ...
              barChart.setPosition(sheetGraph, 1, 22);
              barChart.setSize(500, 280);
              sheetGraph.addChart(barChart);
